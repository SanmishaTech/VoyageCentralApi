...
const TRAVEL_DOCUMENT_MODULE_NAME = "groupBooking/travelDocuments"; // Define module name consistently
...
// Create a new tour
const createGroupClientTravelDocument = async (req, res, next) => {
...
// Get a tour by ID
const getGroupClientTravelDocumentById = async (req, res, next) => {
  const { travelDocumentId } = req.params;

  try {
    const travelDocument = await prisma.travelDocument.findFirst({
      where: {
        AND: [{ id: parseInt(travelDocumentId, 10) }],
      },
    });
...
const updateGroupClientTravelDocument = async (req, res, next) => {
  const schema = z.object({
    description: z
      .string()
      .min(1, "Description cannot be left blank.")
  ...
  // start
  const { travelDocumentId } = req.params;
  const TravelDocumentId = parseInt(travelDocumentId, 10);
  const requestUploadUUID = req.uploadUUID; // UUID for *this* request's potential new uploads

  console.log(
    `[Update Start:${requestUploadUUID}] Updating agency ID: ${TravelDocumentId}`
  );
  console.log(
  ...
  try {
    ...
    const updatedTravelDocument = await prisma.travelDocument.update({
      where: { id: parseInt(TravelDocumentId, 10) },
      data: {
    ...
    if (error.code === "P2025")
      return next(
        createError(404, `Update failed: travel document not found.`)
      );
...
// Delete a tour
const deleteGroupClientTravelDocument = async (req, res, next) => {
  const { travelDocumentId } = req.params;
  const TravelDocumentId = parseInt(travelDocumentId, 10);
...
const getAllTravelDocumentsByGroupClientBookingId = async (req, res) => {
  const { groupClientBookingId } = req.params;
