...
const createGroupClientJourneyBooking = async (req, res, next) => {
  const schema = z
    .object({
...
const getGroupClientJourneyBookingById = async (req, res, next) => {
  const { journeyBookingId } = req.params;

  try {
    const journeyBooking = await prisma.journeyBooking.findUnique({
      where: { id: parseInt(journeyBookingId, 10) },
    });
...
const updateGroupClientJourneyBooking = async (req, res, next) => {
...
const deleteGroupClientJourneyBooking = async (req, res, next) => {
  const { journeyBookingId } = req.params;
...
const getAllJourneyBookingsByGroupClientBookingId = async (req, res, next) => {
  const { groupClientBookingId } = req.params;
  try {
