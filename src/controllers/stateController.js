...
const createState = async (req, res, next) => {
  ...
  try {
    ...
    const newState = await prisma.state.create({
      data: { stateName, countryId, agencyId: req.user.agencyId || null },
    });
  ...
  try {
    ...
    if (
      ...
      return res.status(409).json({
        errors: {
          message:
            "Cannot delete this State because it is referenced in related data. Please remove the related references before deleting.",
        },
      });
    }
    if (error.code === "P2025") {
...
const getStatesOfIndia = async (req, res, next) => {
  try {
    ...
    const india = await prisma.country.findFirst({
      where: {
        countryName: "India",
        agencyId: req.user.agencyId || null, // Ensure the country belongs to the user's agency
      },
      select: { id: true },
    });
