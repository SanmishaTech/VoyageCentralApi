 ...
 */
async function generateVehicleHRVNumber(tx, agencyId) {
  const now = dayjs();
  ...
  const lastBooking = await tx.vehicleBooking.findFirst({
    where: {
      AND: [
        {
          vehicleHrvNumber: {
  ...
  if (lastBooking?.vehicleHrvNumber) {
    const parts = lastBooking.vehicleHrvNumber.split("/");
...
module.exports = generateVehicleHRVNumber;
