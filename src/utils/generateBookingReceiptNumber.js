 ...
 */
async function generateBookingReceiptNumber(tx, agencyId) {
  const now = dayjs();
  ...
  const lastBooking = await tx.bookingReceipt.findFirst({
    where: {
      AND: [
        {
          receiptNumber: {
  ...
  if (lastBooking?.receiptNumber) {
    const parts = lastBooking.receiptNumber.split("/");
...
module.exports = generateBookingReceiptNumber;
