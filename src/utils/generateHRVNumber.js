 ...
 */
async function generateHRVNumber(tx, agencyId) {
  const now = dayjs();
  ...
  const lastBooking = await tx.hotelBooking.findFirst({
    where: {
      AND: [
        {
          hrvNumber: {
  ...
  if (lastBooking?.hrvNumber) {
    const parts = lastBooking.hrvNumber.split("/");
...
module.exports = generateHRVNumber;
